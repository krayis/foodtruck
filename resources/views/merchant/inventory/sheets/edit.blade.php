...
@section('content')
    ...
    <form action="{{ route('merchant.inventory.sheets.update', $sheet->id) }}" method="POST" enctype="multipart/form-data"
          autocomplete="off">
        @csrf
        @method('PATCH')
    ...
    <form action="{{ route('merchant.inventory.sheets.destroy', $sheet->id) }}" style="display: none;" id="delete-form"
          method="POST">
        @csrf
        @method('DELETE')
    </form>
