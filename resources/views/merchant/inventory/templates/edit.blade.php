...
@section('content')
    ...
    <form action="{{ route('merchant.inventory.templates.update', $template->id) }}" method="POST"
    ...
    <form action="{{ route('merchant.inventory.templates.destroy', $template->id) }}" id="form-delete"
          style="display: none;"
          method="POST">
        @csrf
        @method('DELETE')
