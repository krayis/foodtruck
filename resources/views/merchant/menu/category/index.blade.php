...
@section('content')
    ...
    <div class="page-header">
        ...
        <div class="page-action">
            <a class="btn btn-primary" href="{{ route('merchant.menu.category.create') }}"><ion-icon name="add"></ion-icon> New Category</a>
        </div>
    </div>
    ...
    <table class="table">
        ...
        @foreach($categories as $category)
            <tr data-item-id="{{$category->id}}" data-sort-order="{{$category->sort_order}}">
                <td>
                    <a href="{{ route('merchant.menu.category.show', $category->id) }}">{{ $category->name }}</a>
                </td>
                <td align="center">
